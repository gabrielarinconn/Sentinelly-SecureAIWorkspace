...
from backend.presentation.errors import app_error


def get_current_user_id(authorization: str = Header(...)) -> str:
    """El user_id SIEMPRE sale del JWT verificado (regla dura) — nunca del body/query."""
    if not authorization.startswith("Bearer "):
        raise app_error(status.HTTP_401_UNAUTHORIZED, "MISSING_BEARER_TOKEN", "Missing bearer token.")
    token = authorization.removeprefix("Bearer ")
    try:
        return JwtTokenService().decode_access_token(token)
    except InvalidTokenError as exc:
        raise app_error(status.HTTP_401_UNAUTHORIZED, "INVALID_TOKEN", str(exc)) from exc
