...
export function ChatPanel({ channelId }: { channelId: string | null }) {
  const { user } = useAuth();
  const { t } = useI18n();
      ...
      {searchResults !== null ? (
        ...
        <>
          <div className="message-list" ref={listRef}>
            {loadState === "loading" && <div className="state-message">{t("chat.loadingHistory")}</div>}
                ...
                <div key={message.clientId} className={"message" + (isOwn ? " own" : "")}>
                  {editingId === message.id ? (
                    ...
                    <>
                      ...
                      {message.delivery === "pending" && <span className="delivery-tag">{t("chat.deliveryPending")}</span>}
